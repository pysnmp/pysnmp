...
async def bulkWalk(agent, monkeypatch, maxRepetitions=10, **options):
...
class TestWalkStopsAtTheSubtree:
    async def test_rows_outside_the_subtree_are_not_yielded(self, table, monkeypatch):
        # The whole point: PAST_END is lexicographically next, and GETNEXT will
        # happily return it. A walk must not.
        rows = await walk(_Agent(table), monkeypatch)
        assert names(rows) == [oid(1), oid(2), oid(3)]

    async def test_lexicographic_mode_runs_past_the_end(self, table, monkeypatch):
        rows = await walk(_Agent(table), monkeypatch, lexicographicMode=True)
        assert names(rows) == [oid(1), oid(2), oid(3), PAST_END + (1,)]

    async def test_an_empty_subtree_yields_nothing(self, monkeypatch):
        rows = await walk(_Agent([(PAST_END + (1,), Integer(6))]), monkeypatch)
        assert rows == []

    async def test_end_of_mib_ends_the_walk(self, monkeypatch):
        rows = await walk(_Agent([(oid(1), OctetString("eth0"))]), monkeypatch)
        assert names(rows) == [oid(1)]


class TestWalkBounds:
    async def test_max_rows_stops_the_walk(self, table, monkeypatch):
        rows = await walk(_Agent(table), monkeypatch, maxRows=2)
        assert names(rows) == [oid(1), oid(2)]

    async def test_max_calls_stops_the_walk(self, table, monkeypatch):
        agent = _Agent(table)
        rows = await walk(agent, monkeypatch, maxCalls=1)
        assert len(rows) == 1
        assert agent.calls == 1

    async def test_stopping_early_stops_requesting(self, table, monkeypatch):
...
class TestWalkReportsFailure:
    async def test_an_error_is_yielded_before_the_walk_ends(self, table, monkeypatch):
        ...
        rows = await walk(agent, monkeypatch)

        assert len(rows) == 2
        assert rows[0][0] is None
        assert rows[-1][0] is errind.requestTimedOut

    async def test_an_immediate_error_is_still_yielded(self, table, monkeypatch):
    ...
    async def test_a_non_increasing_oid_ends_the_walk_by_default(
    ...
    async def test_ignore_non_increasing_oid_carries_on(self, table, monkeypatch):
        ...
        rows = await walk(agent, monkeypatch, ignoreNonIncreasingOid=True)

        assert all(row[0] is None for row in rows)


class TestBulkWalk:
    async def test_the_overshoot_is_trimmed(self, table, monkeypatch):
        ...
        rows = await bulkWalk(_Agent(table), monkeypatch, maxRepetitions=10)
        assert names(rows) == [oid(1), oid(2), oid(3)]

    async def test_a_single_row_response_past_the_end_stops(self, table, monkeypatch):
        ...
        rows = await bulkWalk(_Agent(table), monkeypatch, maxRepetitions=1)
        assert names(rows) == [oid(1), oid(2), oid(3)]

    async def test_lexicographic_mode_keeps_the_overshoot(self, table, monkeypatch):
        rows = await bulkWalk(
            _Agent(table), monkeypatch, maxRepetitions=10, lexicographicMode=True
        )
        assert PAST_END + (1,) in names(rows)

    async def test_max_rows_bounds_what_is_requested(self, table, monkeypatch):
        # Never make the agent build rows that will be thrown away.
        agent = _Agent(table)
        rows = await bulkWalk(agent, monkeypatch, maxRepetitions=10, maxRows=2)
        assert len(rows) == 2
        assert agent.maxRepetitionsSeen == [2]

    async def test_an_error_is_yielded(self, table, monkeypatch):
        agent = _Agent(table, errorAt=1, errorIndication=errind.requestTimedOut)
        rows = await bulkWalk(agent, monkeypatch)
        assert len(rows) == 1
        assert rows[0][0] is errind.requestTimedOut


class TestYieldShape:
    async def test_a_row_unpacks_positionally(self, table, monkeypatch):
        errorIndication, errorStatus, errorIndex, varBinds = (
            await walk(_Agent(table), monkeypatch)
        )[0]
        assert errorIndication is None
        assert errorIndex == 0
        assert tuple(varBinds[0][0]) == oid(1)

    async def test_a_row_also_has_named_fields(self, table, monkeypatch):
        row = (await walk(_Agent(table), monkeypatch))[0]
