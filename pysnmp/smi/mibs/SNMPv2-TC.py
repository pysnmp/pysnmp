    ...
    def prettyOut(self, value):  # override asn1 type method
        ...
        if self.displayHint and (
            ...
            while runningValue and displayHint:
                # 1
                if displayHint[0] == "*":
                    repeatIndicator = repeatCount = runningValue[
                        0
                    ]  # oct2int: bytes element is already int in Python 3
...
class RowStatus(TextualConvention, Integer):
    ...
    def setValue(self, value):
        ...
        debug.logger & debug.flagIns and debug.logger(
            f"RowStatus state change from {self!r} to {value!r} produced new state {newState!r}, error indication {excValue!r}"
        )

        if excValue is not None:
            excValue = excValue(
                msg=f"Exception at row state transition from {self!r} to {value!r} yields state {newState!r} and exception",
                syntax=newState,
            )
            raise excValue
