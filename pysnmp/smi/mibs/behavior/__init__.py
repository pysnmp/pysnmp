...
- keep it idempotent. pysmi 4.0.0 stopped splicing its own copy of these
  fragments into the modules it bundles (pysnmp/pysmi#245), so on 4.x this is
  the only source. A module generated by a 3.x pysmi still carries pysmi's
  copy, and then both run, this one last -- which is what idempotence buys.
"""
