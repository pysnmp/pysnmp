...
def test_walk_next_node_names(benchmark, mib_view_controller):
    ...
    def _():
        ...
        for _unused in range(32):
            nodeName, _label, _suffix = mib_view_controller.getNextNodeName(nodeName)


def test_object_identity_resolve_by_symbol(benchmark, mib_view_controller):
    @benchmark
    def _():
        rfc1902.ObjectIdentity("SNMPv2-MIB", "sysDescr", 0).resolveWithMib(
            mib_view_controller
        )
...
def test_object_identity_resolve_by_oid(benchmark, mib_view_controller):
    @benchmark
    def _():
        rfc1902.ObjectIdentity("1.3.6.1.2.1.1.5.0").resolveWithMib(mib_view_controller)
